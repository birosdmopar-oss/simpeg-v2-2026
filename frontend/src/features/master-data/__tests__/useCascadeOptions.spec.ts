...
const data: Record<string, MasterOption[]> = {
  ...
  'kecamatan|3171': [{ id: '3171010', nama: 'Gambir', parent: '3171' }],
}
...
describe('useCascadeOptions', () => {
  ...
  it('memilih level memuat level berikutnya dari induk terpilih', async () => {
    ...
    expect(c.levels.value[2]?.options.map((o) => o.id)).toEqual(['3171010'])
    await c.select(2, '3171010')
    expect(c.leafValue()).toBe('3171010')
  })

  it('mengganti level atas mengosongkan level di bawahnya', async () => {
    const c = useCascadeOptions(ref(chain))
    await c.init(['31', '3171', '3171010'])
    expect(c.leafValue()).toBe('3171010')
    await c.select(0, '')
...
describe('resolveAncestorPath', () => {
  it('menelusuri induk ke atas dari id induk langsung', async () => {
    const rows: Record<string, MasterRow> = {
      'kecamatan/3171010': { id_kecamatan: '3171010', id_kabupaten_kota: '3171', order: 1, status: '1' },
    ...
    expect(await resolveAncestorPath(chain, '3171010')).toEqual(['31', '3171', '3171010'])
  })
})
